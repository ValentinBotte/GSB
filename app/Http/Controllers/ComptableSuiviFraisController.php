...
class ComptableSuiviFraisController extends Controller
{
    ...
    public function getMois(){
        ...
        $lesMois = DB::table('fichefrais')->select('mois')->where('idvisiteur', '=', Input::get('idVisiteur'))->where('idetat', '=', 'VA')->where('idvisiteur', '=', Input::get('idVisiteur'))->orderBy('mois', 'desc')->get();
    ...
    public function utilitaire(){
        ...
        $lesMois = DB::table('fichefrais')->select('mois')->orderBy('mois', 'desc')->get();
        foreach($lesMois as $unMois){
            if(substr($unMois->mois, 0, 4) >= date('Y') - 1){
                $tempMois = substr($unMois->mois, 4, 6) . '/' . substr($unMois->mois, 0, 4);
                $afficheMois[] = $tempMois;
